package exercise_25_1;

/* Author: Renee Linford
 * Date: 10-24-19
 * ADS Exercise 25-1: Add new methods in BST
 */

// BST article: https://www.cs.bu.edu/teaching/c/tree/breadth-first/
...
public class Exercise25_1 {
	...
	public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E> {
		 ...
		 */
		public int height() {
			// Default method for height.
			
			TreeNode<E> node = this.root; // Create node to pass to recursive call.
			if (node == null) // If node is null, return 0 & end method.
				return 0;
			else
				return height(node); // Recursive call.
		}
		...
		public int height(TreeNode<E> node) {
			...
			if (node == null) // If node is null, return 0 & end method.
				...
				if (height(node.left) > height(node.right)) // Compare left child & right child.
					return height(node.left) + 1; // Recursive call for left child to find its children.
				else
					return height(node.right) + 1; // Recursive call for right child to find its children.
			}
		}
		...
		public void breadthFirstTraversal() {
			// Create linked list to compile the traversal.
			// Tracks the nodes whose second child we will investigate. 
			java.util.LinkedList<TreeNode<E>> nodeList = new java.util.LinkedList<TreeNode<E>>();

			TreeNode<E> current = (TreeNode<E>) this.root; // Start at root node.

			if (current == null) // If node is null, end method.
				return;
			else 
				nodeList.add(current); // If not null, add to nodeList.
			while (!nodeList.isEmpty()) { 
				// While nodes are in list pop first item & assign it to current.
				current = nodeList.removeFirst();
				if (current.left != null) // If current as left child, add to list.
					nodeList.add(current.left);
				if (current.right != null) // If current as right child, add to list.
					nodeList.add(current.right);
				System.out.print(current.element + " "); // Print current element.
			}

...
/* ADS Exercise 28-7: Find a cycle in an Unweighted Graph */
...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getACycle() { 
		...
		while (!vList.isEmpty()) { // Check for unvisited vertices. 
			
			// Pick one, call it v.
			int v = vList.size() - 1;

			// Create stack & push v to the stack. 
			...
			while (!stack.isEmpty()) { // While stack is not empty:
				...
				if (neighbors.get(x).isEmpty()) { 
					// If x has no neighbors, pop a vertex from the stack;
					stack.pop();
				}
					...
					for (int i = 0; i < neighbors.get(x).size();  i++) {
						...
						if (isVisited.get(e.v) == false) { // If ending vertex of e (e.v) is not visited: 
							parent[e.v] = x; // Mark parent of ending vertex as x.
							stack.push(e.v); // Push e.v onto stack.
							isVisited.set(e.v, true); // Mark e.v as visited.
							vList.remove(e.v); // Remove e.v from list with all vertices.
							neighbors.get(x).remove(i); // Remove loop variable i from x's neighbor list.
							break;
						} 
						
						else if (e.v != parent[x]) { // If e.v is not x's parent:
							ArrayList<Integer> intArrayList = new ArrayList<>(); // Create an Integer ArrayList.
							intArrayList.add(e.v); // Add e.v to list.
							
							while ((x != e.v) && (x != -1)) { 
								intArrayList.add(x); // Add x to list.
								x = parent[x]; // Mark x as parent of x.
							}
							return intArrayList;
						}
						else { 
		...
		return new ArrayList<>(); // Else return an empty list.
	}

...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		...
		// New ArrayList to hold path.
		ArrayList<Integer> path = new ArrayList<>();
		
		int index = v; // Variable index to check.
		
		SearchTree tree = bfs(u); // Build tree using breadth first search using starting point (u).
		
		while (index != -1 && index != u) {
			// If index is not the root or equal to u.
			
			path.add(index); // Add index to path list.
			index = tree.getParent(index); // Change index to equal parent. (traverse tree from child to parent)
			
			if (index == u) { // If index equals u (start point)
				path.add(u); // Add u to path list.
			}
		}
		return path; // Return Integer List.
	}
